...
func TestTimes_Basic(t *testing.T) {
	...
	expected := Slice[int]{9, 18, 27, 36, 45, 54, 63, 72, 81, 90}
...
func TestTimes_Zero(t *testing.T) {
	...
	if len(c) != 0 {
		t.Fatalf("expected empty collection, got %v", c)
	}
}
...
func TestTimes_Large(t *testing.T) {
	...
	if len(c) != 1000 {
		t.Fatalf("expected 1000 items, got %d", len(c))
	}

	// spot check
	if c[0] != 1 || c[999] != 1000 {
		t.Fatalf("indexing mismatch: first=%v last=%v", c[0], c[999])
	}
}
